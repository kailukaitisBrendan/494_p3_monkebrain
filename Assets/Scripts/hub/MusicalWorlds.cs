...
public class MusicalWorlds : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (wagon.transform.position.x < 75f && wagon.transform.position.x > -10f) {
            d.SetActive(false);
            b.SetActive(true);
            if (a.activeSelf) {
        ...
        if (wagon.transform.position.x < -10f && wagon.transform.position.x > -80f) {
            a.SetActive(false);
            c.SetActive(true);
            if (d.activeSelf) {
                //if (b.GetComponent<AudioSource>().volume > 0.79f)
            ...
            if (c.GetComponent<AudioSource>().volume < 0.20f)
                c.GetComponent<AudioSource>().volume += Time.deltaTime * 0.5f;
        }
